app.controller("usersController", function ($scope, userService, toastr, errorHandler) {
    $(".content").slimscroll({
        height: window.innerHeight - 135,
        alwaysVisible: true,
        size: "3px"
    });
    userService.$resource().query({}, function success(users) {
        $scope.users = users;
    }, errorHandler.handleError);
});
app.controller("userController", function ($scope, $routeParams, $location, userService, toastr, errorHandler, roleService) {
    ...
    function init() {
        ...
        userService.$resource().get({ userId: $routeParams.id }, function (user) {
            $scope.user = user;
        }, errorHandler.handleError);
        getUsersRoles();
    }
    function getUsersRoles() {
        userService.$resource().getRoles({ userId: $routeParams.id }, function (userRoles) {
    ...
    $scope.updateUserDetails = function (form) {
        if ($routeParams.id === "new") {
            ...
            $scope.savingUser = true;
            $scope.user.$update({ userId: $routeParams.id }, function success(user) {
                $scope.savingUser = false;
                setTimeout(function () {
                    $scope.$apply();
    ...
    $scope.resetUserPassword = function (form) {
        ...
        user.$resetPassword({}, function success(user) {
    ...
    $scope.assignRole = function () {
        ...
        userToRole.$addRole({}, function success() {
    ...
    $scope.removeRole = function (role) {
        ...
        $scope.user.$removeRole({}, function success() {
    ...
    $scope.filterRoles = function (value) {
        var searchVal = false;
        angular.forEach($scope.userRoles, function (role, key) {
