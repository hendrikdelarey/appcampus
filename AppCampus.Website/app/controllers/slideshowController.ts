app.controller("slideshowsController", function ($scope, $log, $modal, $location, devicesService, slideshowService, slideService, errorHandler) {
    ...
    $scope.deleteSlideshow = function (slideshow) {
        ...
        modalInstance.result.then(function (confirmation) {
            ...
            slideshow.$delete({ slideshowId: slideshow.slideshowId },
                function (deletedSlideshow) {
                    loadSlideshows();
...
app.controller("slideshowController", function ($scope, $log, $routeParams, $modal, $location, toastr, slideshowService, slideService, widgetService, widgetDefinitionService, parameterDefinitionService, parameterService, errorHandler) {
    ...
    function loadParameters() {
        ...
        parameterDefinitionService.query({ widgetDefinitionId: $scope.selectedSlideWidget.widgetDefinitionId }).
            ...
            function (parameterDefinitions)
            {
                ...
                angular.forEach($scope.parameterDefinitions, (value, key) => {
    ...
    function findOperatorName(): string {
        var returnValue = "Not Set";
        angular.forEach($scope.parameterDefinitions,(pdef, key) => {
    ...
    $scope.updateslideshowName = function (form) {
        ...
        $scope.slideshow.$update({ slideshowId: $scope.slideshow.slideshowId},
            function success() {
    ...
    $scope.createNewSlide = function (slidesForm) {
        if (slidesForm.$invalid) {
            //toastr.error("Please fill out the slide name", "Validation error");
            return false;
        }
        if (typeof ($scope.newSlideName) !== "undefined" && $scope.newSlideName.trim() !== "") {
            ...
            slide.$save({ slideshowId: $routeParams.id },
    ...
    $scope.moveSlideUp = function (index, slide) {
        if (index - 1 >= 0) {
            ...
            $scope.actionHappening = true;
            $scope.slideActivity = true;
            $scope.slideshow.$move({ slideshowId: $routeParams.id },
                function success(updatedSlide) {
    ...
    $scope.removeSlide = function (slide, callback) {
        ...
        modalInstance.result.then(function (confirmation) {
            ...
            slide.deleting = true;
            $scope.actionHappening = true;
            slide.$delete({ slideshowId: $routeParams.id, slideId: slide.slideId },
                function success() {
                    $scope.alreadyHasWidget = false;
    ...
    $scope.updateSlideDetails = function () {
        ...
        $scope.selectedSlide.$update({ slideshowId: $routeParams.id, slideId: $scope.selectedSlide.slideId },
            function success() {
                ...
                if (!$scope.alreadyHasWidget && typeof ($scope.selectedSlideWidget) !== "undefined") {
                    ...
                    $scope.selectedSlideWidget.$save({ slideshowId: $routeParams.id, slideId: $scope.selectedSlide.slideId },
                        (widget) => {
                            $scope.alreadyHasWidget = true;
                            loadParameters();                    
                        },
    ...
    $scope.editParameter = function (parameterName: string, parameterDefinitionValue: string) {
        var operatorName = "";
        ...
        if (parameterName === "StopIdentifier") {
            if (findOperatorName() == "Not Set") {
                ...
                angular.forEach($scope.parameterDefinitions,(parameter, index) => {
        ...
        modalInstance.result.then(function (newName) {
            ...
            param.$save({ slideshowId: $routeParams.id, slideId: $scope.selectedSlide.slideId, widgetId: $scope.selectedSlideWidget.widgetId },
...
app.controller("updateParameterModal", function ($scope, $timeout, $log, $modalInstance, toastr, stopsDefinitionService, parameterName, parameterDefinitionValue, operatorName, imageService, operatorService, errorHandler) {
    ...
    switch ($scope.parameterName) {
        case "StopIdentifier":
            ...
            $scope.stops = stopsDefinitionService.query({},
                (stops) => {
                    $scope.stops = stops;
    ...
    $scope.saveParameter = function (value: string) {
