app.controller("signboardsController", function ($scope, signboardsService, $filter, $http, userService, toastr, dateFormatter, diagnosticService, scheduledSlideshowService, slideshowService, errorHandler) {
    ...
    function getSignboards() {
        var signboards = signboardsService.query()
            ...
            angular.forEach($scope.signboards, function (value, index) {
                diagnosticService.latest({ signboardId: value.signboardId }).$promise.then(function (signboardDiagnostics) {
            ...
            angular.forEach($scope.signboards, function (signboard, index) {
                scheduledSlideshowService.query({ "signboardId": signboard.signboardId }).$promise.then(function success(scheduledSlideshows) {
                    var latestDate = new Date(1999, 12, 31);
                    angular.forEach(scheduledSlideshows, function (scheduledSlideshow, key) {
    ...
    function getActiveSlideshowName(signboard) {
        slideshowService.get({ "slideshowId": signboard.activeSlideshowId }, function success(slide) {
...
app.controller("signboardController", function ($scope, $routeParams, $log, $filter, $modal, signboardsService, slideshowService, scheduledSlideshowService, announcementService, assignedSignboardsService, diagnosticService, dateFormatter, errorHandler, toastr, softwareService) {
    init();
    function init() {
        $(".cpu").knob({ width: 5 });
        $(".disk-space").knob();
    ...
    function getScheduledSlideshows() {
        var schedulesSlideShows = scheduledSlideshowService.query({ "signboardId": $routeParams.id }).
            ...
            angular.forEach(scheduledSlideshows, function (scheduledSlideshow, index) {
    ...
    function getAnnouncements() {
        announcementService.query().
            $promise.then(function (announcements) {
    ...
    function getDiagnostics() {
        diagnosticService.latest({ "signboardId": $routeParams.id }).
            $promise.then(function success(diagnostics) {
            setTimeout(function () {
    ...
    function getSlideshows() {
        slideshowService.query().
            $promise.then(function success(slideshows) {
    ...
    $scope.removeAnnouncement = function (announcement) {
        ...
        modalInstance.result.then(function (confirmation) {
            ...
            assignedSignboardsService.query({ announcementId: announcement.announcementId }).
                ...
                angular.forEach(assignedSignboards, function (signboard, index) {
                    if (signboard.signboardId === $routeParams.id) {
                        assignedSignboards.splice(index, 1);
                        assignedSignboardsService.update({ announcementId: announcement.announcementId }, assignedSignboards, function success() {
                            getAnnouncements();
    ...
    $scope.scheduleSlideshow = function (form) {
        ...
        if (typeof ($scope.scheduledSlideshow.slideshowId) !== "undefined") {
            ...
            new scheduledSlideshowService({ slideshowId: $scope.scheduledSlideshow.slideshowId, startDate: new Date($scope.scheduledSlideshow.startDate).toISOString() }).
                $save({ signboardId: $routeParams.id }, function success() {
                getScheduledSlideshows();
    ...
    $scope.unscheduleSlideshow = function (slideshow) {
        ...
        modalInstance.result.then(function (confirmation) {
            ...
            slideshow.$delete({ signboardId: $routeParams.id, slideshowId: slideshow.slideshowId, scheduledSlideshowId: slideshow.scheduledSlideshowId }, function success() {
    ...
    $scope.setFontSizeFactor = function (form) {
        ...
        signboardRequest.$request({ signboardId: $routeParams.id }, function success(requests) {
    ...
    $scope.restartDevice = function () {
        ...
        sr.$request({ signboardId: $routeParams.id }, function success(requests) {
    ...
    $scope.getDiagnostic = function (metricName) {
        ...
        angular.forEach($scope.diagnostics.metrics, function (metric, key) {
