app.controller("announcementsController", function ($scope, $log, $modal, announcementService, dateFormatter, errorHandler) {
    ...
    $scope.enableAnnouncement = function (announcement) {
        ...
        announcement.$update({ announcementId: announcement.announcementId },
            function success() { },
            function error(error) {
                errorHandler.handleError(error);
            });
    ...
    $scope.deleteAnnouncement = function (announcement) {
        ...
        modalInstance.result.then(function (confirmation) {
            ...
            announcement.$delete({ announcementId: announcement.announcementId },
...
app.controller("announcementController", function ($scope, $routeParams, $location, $log, signboardsService, assignedSignboardsService, announcementService, dateFormatter, toastr, errorHandler) {
    ...
    function loadAnnouncement() {
        announcementService.get({ "announcementId": $routeParams.id })
            .$promise.then(function (data) {
                //data.startDate = dateFormatter.formatDate(data.startDate);
                //data.endDate = dateFormatter.formatDate(data.endDate);
    ...
    function loadAssignedSignboards() {
        ...
        assignedSignboardsService.query({ announcementId: $scope.announcement.announcementId }).
            ...
            function (assignedSignboards) {
                ...
                angular.forEach(assignedSignboardsIds, (value, key) => {
                    signboardsService.get({ "signboardId": value.signboardId }).$promise.then(
                        function (signboard) {
    ...
    function pushIfNotExists(signboard) {
        var canPush = true;
        angular.forEach($scope.assignedSignboards,(value, key) => {
    ...
    function getIndex(signboardId) {
        var returnValue;
        angular.forEach($scope.assignedSignboardIds, (value, key) => {
    ...
    function getIndexOfSignboard(signboardName) {
        var returnVal:any = {};
        angular.forEach($scope.signboards,(signboard, key) => {
    ...
    $scope.saveAnnouncement = function (announcementForm) {
        ...
        if ($routeParams.id === "new") {
            ...
            saveAnnouncement.$update({ 'announcementId': $scope.announcement.announcementId },
                function success() { $scope.announcement = saveAnnouncement; toastr.success("Announcement updated", "Success"); $scope.creatingAnnouncement = false; },
                function error(error) { errorHandler.handleError(error); $scope.creatingAnnouncement = false; });
        }
    };
    ...
    $scope.assignThisAnnouncementToSelectedSignboard = function () {
        if (typeof ($scope.selectedSignboard) !== "undefined") {
            if ($routeParams.id !== "new") {
                ...
                angular.forEach(newBoards, (value, index) => {
                    delete value.name;
                    delete value.macAddress;
                });
                                
                angular.forEach($scope.selectedSignboard,(value, index) => {
                ...
                assignedSignboardsService.update({ "announcementId": $scope.announcement.announcementId }, newBoards,
                    function success() {
    ...
    $scope.unassignThisAnnouncementFromSelectedSignboard = function (signboard, index) {
        ...
        assignedSignboardsService.update({ "announcementId": $scope.announcement.announcementId }, $scope.assignedSignboardIds,
            function success(assignedSignboards) {
    ...
    $scope.assignedSignboardsFilter = function (value) {
        var searchVal = false;
        angular.forEach($scope.assignedSignboards,(signboard, key) => {
